...
- [x] 레지스트리 정리 스크립트 작성 및 실행 (scripts/registry-cleanup.ps1)
...
- [ ] 레지스트리 정리 (핵심 기능)
  - 삭제된 프로그램의 Uninstall 고아 키 감지 및 제거
  - SOFTWARE 하위 잔여 키 감지 (키워드 매칭)
  - 시작프로그램에 남은 잔여 항목 감지 및 제거
  - 존재하지 않는 경로를 참조하는 셸 확장 감지
  - 안전도 표시: "시스템 키는 건드리지 않아요" 안내
  - 정리 전/후 비교 리포트 ("12개 정리됨, Explorer가 빨라집니다")
